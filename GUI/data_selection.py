...
dir = 'Data/' + event + '/'
seislist = glob.glob(direc+'/*PICKLE') 
...
for s in range(len(seislist)):
    ...
    while answer != 'y' and answer != 'n':
        ...
        plt.subplot(2,1,1)       
        plt.plot(seisT.times()+tshift,seisT.data/np.max(seisT.data),'k', linewidth=3)
        ...
        if syn == True:
             ...
             plt.subplot(2,1,2)
             plt.plot(synR.times(),synR.data/np.max(seisT.data),color=[0.5,0.5,0.5])
             plt.subplot(2,1,1)
             plt.plot(synT.times(),synT.data/np.max(seisT.data),color=[0.5,0.5,0.5])
        ...
        answer=input('want this seismogram (y/n)?')
 
        if (answer=='y'):
            print('data accepted')
        if (answer=='n'):
            shutil.move(seislist[s],dirdump)
        # here an answer == 'p' could be implemented to switch to polarity.
